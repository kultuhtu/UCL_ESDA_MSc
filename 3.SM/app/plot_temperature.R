...
plot_electricity_consumption <- function(df, 
  ...
  electricity_consumption_plot_theme <- theme(
    ...
    legend.title =  element_text(colour = "#485785"),
  ...
  if (input_filter == "Day of Month") {
    ...
    plot <- ggplot(df, aes(x = day, y = kwh_per_hh_sum, color = Acorn_grouped)) + 
      ...
      labs(x = "Day of Month",
           ...
           color = "Group") + electricity_consumption_plot_theme
    ...
    plot <- ggplot(df, aes(x = week, y = kwh_per_hh_sum, 
      ...
      labs(x = "Week",  
           y = "Average Weekly Electricity Consumption (kWh)",
           color = "Group") + electricity_consumption_plot_theme
    ...
    plot <- ggplot(df, aes(x = month, y = kwh_per_hh_sum, 
      ...
      labs(x = "Month",  
           y = "Average Monthly Electricity Consumption (kWh)",
           color = "Group") + electricity_consumption_plot_theme
  }
  return(plot)
}