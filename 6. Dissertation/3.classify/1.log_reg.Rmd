...
train %>% sample_frac(0.1) %>% vis_dat()
val %>% sample_frac(0.1) %>% vis_dat()
test %>% sample_frac(0.1) %>% vis_dat()
```
...
```{r}
# logistic
log <- glm(state ~., data = train, family = binomial(link = "logit"))

log_pred_val <- log %>% predict(X_val,type="response")
head(log_pred_val, 10)
```

```{r}
log_pred_val <- ifelse(log_pred_val > 0.5, 1, 0)
...
```{r}
actual <- as.factor(y_val$state)
pred <- as.factor(log_pred_val)
...
```{r}
log_pred_test <- log %>% predict(X_test,type="response")
head(log_pred_test, 10)
```

```{r}
log_pred_test <- ifelse(log_pred_test > 0.5, 1, 0)
...
```{r}
actual <- as.factor(y_test$state)
pred <- as.factor(log_pred_test)
...
cm_d$classes <- 2
cm_d$model <- "Logistic Regression"

png(paste0("LR_cm1.png"), width = 125, height = 75, units='mm', res = 700)
...
```{r}
# logistic
log_bal <- glm(state ~., data = over, family = binomial(link = "logit"))

log_bal_pred_val <- log_bal %>% predict(X_val,type="response")
head(log_bal_pred_val, 10)
```

```{r}
log_bal_pred_val <- ifelse(log_bal_pred_val > 0.5, 1, 0)

head(log_bal_pred_val, 10)
mean(log_bal_pred_val == y_val$state)
```

```{r}
actual <- as.factor(y_val$state)
pred <- as.factor(log_bal_pred_val)
...
```{r}
log_bal_pred_test <- log_bal %>% predict(X_test,type="response")
head(log_bal_pred_test, 10)
```

```{r}
log_bal_pred_test <- ifelse(log_bal_pred_test > 0.5, 1, 0)

head(log_bal_pred_test, 10)
mean(log_bal_pred_test == y_test$state)
```

```{r}
actual <- as.factor(y_test$state)
pred <- as.factor(log_bal_pred_test)
...
png(paste0("LR_cm2.png"), width = 125, height = 75, units='mm', res = 700)
...
```{r}
plot.roc(y_val$state,
         log_bal_pred_val, col = "blue",
         lwd = 3, print.auc = TRUE, print.auc.y = 0.3,
         main = "ROC-AUC of LR")
```

```{r}
log_imp <- varImp(log_bal)
print(log_bal)
summary(log_bal)
```

```{r}
a <- varImp(log_bal, scale = FALSE)
```
