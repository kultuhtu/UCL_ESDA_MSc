...
df_heat <- read_csv(
  "0.data/sensor/home70_hall773_sensor2293_tempprobe_central-heating-flow.csv.gz", show_col_types = FALSE)
df_gas <- read_csv(
  "0.data/sensor/home70_hall774_sensor2304_gas-pulse_gas.csv.gz", 
  show_col_types = FALSE)

df_tbath <- read_csv(
  "0.data/auxil/home70_bathroom776_sensor2317_room_temperature.csv.gz", show_col_types = FALSE)
df_tbed1 <- read_csv(
  "0.data/auxil/home70_bedroom779_sensor2339_room_temperature.csv.gz", show_col_types = FALSE)
df_tbed2 <- read_csv(
  "0.data/auxil/home70_bedroom778_sensor2327_room_temperature.csv.gz", show_col_types = FALSE)
df_thall <- read_csv(
  "0.data/auxil/home70_bedroom780_sensor2334_room_temperature.csv.gz", show_col_types = FALSE)
df_tkitch <- read_csv(
  "0.data/auxil/home70_study775_sensor2312_room_temperature.csv.gz", show_col_types = FALSE)
df_tlr <- read_csv(
  "0.data/auxil/home70_kitchenliving777_sensor2321_room_temperature.csv.gz", show_col_types = FALSE)
...
df_gas_main <- as.data.table(df_gas_filtered)[as.data.table(df_nul), on = .(date), roll = -740]
...
```{r results = 'hold'}
df_tbath_clean <- df_tbath_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),197,.)))
df_tbed1_clean <- df_tbed1_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),205,.)))
df_tbed2_clean <- df_tbed2_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),192,.)))
df_thall_clean <- df_thall_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),202,.)))
df_tkitch_clean <- df_tkitch_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),185,.)))
df_tlr_clean <- df_tlr_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),189,.)))
...
```{r}
# save main outputs
write.csv(df_gas_clean,"outputs/70_gas_clean.csv", row.names = FALSE)
write.csv(df_heat_clean,"outputs/70_heat_clean.csv", row.names = FALSE)
write.csv(df_tavg_clean,"outputs/70_tavg_clean.csv", row.names = FALSE)
write.csv(df_tlr_clean,"outputs/70_tlr_clean.csv", row.names = FALSE)
```
