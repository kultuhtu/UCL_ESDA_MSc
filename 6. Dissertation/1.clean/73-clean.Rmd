...
df_heat <- read_csv(
  "0.data/sensor/home73_kitchenliving781_sensor2346_tempprobe_central-heating-flow.csv.gz", show_col_types = FALSE)
df_gas <- read_csv(
  "0.data/sensor/home73_utility783_sensor2374_gas-pulse_gas.csv.gz", 
  show_col_types = FALSE)

df_tbath <- read_csv(
  "0.data/auxil/home73_bathroom787_sensor2396_room_temperature.csv.gz", show_col_types = FALSE)
df_tbed1 <- read_csv(
  "0.data/auxil/home73_bedroom784_sensor2381_room_temperature.csv.gz", show_col_types = FALSE)
df_tbed2 <- read_csv(
  "0.data/auxil/home73_bedroom786_sensor2391_room_temperature.csv.gz", show_col_types = FALSE)
df_thall <- read_csv(
  "0.data/auxil/home73_hall782_sensor2371_room_temperature.csv.gz", show_col_types = FALSE)
df_tkitch <- read_csv(
  "0.data/auxil/home73_kitchenliving781_sensor2358_room_temperature.csv.gz", show_col_types = FALSE)
df_tlr <- read_csv(
  "0.data/auxil/home73_livingroom785_sensor2386_room_temperature.csv.gz", show_col_types = FALSE)
...
```{r results = 'hold'}
df_tbath_clean <- df_tbath_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),254,.)))
df_tbed1_clean <- df_tbed1_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),188,.)))
df_tbed2_clean <- df_tbed2_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),185,.)))
df_thall_clean <- df_thall_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),201,.)))
df_tkitch_clean <- df_tkitch_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),192,.)))
df_tlr_clean <- df_tlr_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),197,.)))
...
```{r}
# save main outputs
write.csv(df_gas_clean,"outputs/73_gas_clean.csv", row.names = FALSE)
write.csv(df_heat_clean,"outputs/73_heat_clean.csv", row.names = FALSE)
write.csv(df_tavg_clean,"outputs/73_tavg_clean.csv", row.names = FALSE)
write.csv(df_tlr_clean,"outputs/73_tlr_clean.csv", row.names = FALSE)
```
