...
df_heat <- read_csv(
  "0.data/sensor/home66_other753_sensor2091_tempprobe_central-heating-flow.csv.gz", show_col_types = FALSE)
df_gas <- read_csv(
  "0.data/sensor/home66_utility745_sensor2036_gas-pulse_gas.csv.gz", 
  show_col_types = FALSE)

df_tbath <- read_csv(
  "0.data/auxil/home66_bathroom750_sensor2067_room_temperature.csv.gz", show_col_types = FALSE)
df_tbed1 <- read_csv(
  "0.data/auxil/home66_bedroom751_sensor2071_room_temperature.csv.gz", show_col_types = FALSE)
df_tbed2 <- read_csv(
  "0.data/auxil/home66_bedroom752_sensor2077_room_temperature.csv.gz", show_col_types = FALSE)
df_thall <- read_csv(
  "0.data/auxil/home66_hall749_sensor2062_room_temperature.csv.gz", show_col_types = FALSE)
df_tkitch <- read_csv(
  "0.data/auxil/home66_kitchen748_sensor2057_room_temperature.csv.gz", show_col_types = FALSE)
df_tlr <- read_csv(
  "0.data/auxil/home66_livingroom747_sensor2052_room_temperature.csv.gz", show_col_types = FALSE)
...
```{r results = 'hold'}
df_tbath_clean <- df_tbath_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),191,.)))
df_tbed1_clean <- df_tbed1_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),202,.)))
df_tbed2_clean <- df_tbed2_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),191,.)))
df_thall_clean <- df_thall_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),191,.)))
df_tkitch_clean <- df_tkitch_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),229,.)))
df_tlr_clean <- df_tlr_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),206,.)))
...
```{r}
# save main outputs
write.csv(df_gas_clean,"outputs/66_gas_clean.csv", row.names = FALSE)
write.csv(df_heat_clean,"outputs/66_heat_clean.csv", row.names = FALSE)
write.csv(df_tavg_clean,"outputs/66_tavg_clean.csv", row.names = FALSE)
write.csv(df_tlr_clean,"outputs/66_tlr_clean.csv", row.names = FALSE)
```
