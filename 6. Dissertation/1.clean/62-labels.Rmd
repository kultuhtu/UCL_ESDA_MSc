...
```{r}
df_gas <- read_csv("outputs/62_gas_clean.csv", show_col_types = FALSE)
df_heat <- read_csv("outputs/62_heat_clean.csv", show_col_types = FALSE)
df_tlr <- read_csv("outputs/62_tlr_clean.csv", show_col_types = FALSE)
df_tavg <- read_csv("outputs/62_tavg_clean.csv", show_col_types = FALSE)
```
...
ggplot() +
  ...
  geom_line(data=df_heat_small, aes(x=date, y=t), 
            alpha=0.7,  color="red") +
  geom_line(data=df_tlr_small, aes(x=date, y=T*1), 
            alpha=0.7,  color="blue") +
  geom_line(data=df_tavg_small, aes(x=date, y=T*1), 
...
df_gas_plot <- df_gas_small %>%
  filter_time('2017-01-20' ~ '2017-01-20')

df_heat_plot <- df_heat_small %>%
  filter_time('2017-01-20' ~ '2017-01-20')

df_tlr_plot <- df_tlr_small %>%
  filter_time('2017-01-20' ~ '2017-01-20')

df_tavg_plot <- df_tavg_small %>%
  filter_time('2017-01-20' ~ '2017-01-20')
```
...
ggplot() +
  ...
  geom_line(data=df_tavg_plot, aes(x=date, y=(T/10), color="Average Indoors Temperature"), alpha=1, size=0.8) +
...
```{r}
ggsave('temp.png', plot = last_plot(), dpi = 800, width = 6, height = 3)
```
...
```{r}
write.csv(df_main,"outputs/62_main.csv", row.names = FALSE)
```
