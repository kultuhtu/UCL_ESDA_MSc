...
df_heat <- read_csv(
  "0.data/sensor/home63_utility727_sensor1878_tempprobe_central-heating-flow.csv.gz", show_col_types = FALSE)
df_gas <- read_csv(
  "0.data/sensor/home63_outside719_sensor1845_gas-pulse_gas.csv.gz", 
  show_col_types = FALSE)

df_tbath <- read_csv(
  "0.data/auxil/home63_bathroom723_sensor1874_room_temperature.csv.gz", show_col_types = FALSE)
df_tbed1 <- read_csv(
  "0.data/auxil/home63_bedroom724_sensor1881_room_temperature.csv.gz", show_col_types = FALSE)
df_tbed2 <- read_csv(
  "0.data/auxil/home63_bedroom725_sensor1869_room_temperature.csv.gz", show_col_types = FALSE)
df_tkitch <- read_csv(
  "0.data/auxil/home63_kitchen720_sensor1851_room_temperature.csv.gz", show_col_types = FALSE)
df_tlr <- read_csv(
  "0.data/auxil/home63_livingroom721_sensor1856_room_temperature.csv.gz", show_col_types = FALSE)
...
df_gas_plot <- df_gas_filtered %>%
  ...
  filter_time('2017-11-10' ~ '2017-12-10')

df_heat_plot <- df_heat_filtered %>%
  filter_time('2017-01-01' ~ '2018-06-01')

df_tlr_plot <- df_tlr_filtered %>%
  filter_time('2017-11-01' ~ '2017-11-10')
...
```{r results = 'hold'}
df_tbath_clean <- df_tbath_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),177,.)))
df_tbed1_clean <- df_tbed1_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),176,.)))
df_tbed2_clean <- df_tbed2_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),172,.)))
df_tkitch_clean <- df_tkitch_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),176,.)))
df_tlr_clean <- df_tlr_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),187,.)))
...
df_tavg_clean$T <- (df_tbath_clean$T + df_tbed1_clean$T + df_tbed2_clean$T 
                     + df_tkitch_clean$T + df_tlr_clean$T)/6
df_tavg_clean
```
...
```{r}
# save main outputs
write.csv(df_gas_clean,"outputs/63_gas_clean.csv", row.names = FALSE)
write.csv(df_heat_clean,"outputs/63_heat_clean.csv", row.names = FALSE)
write.csv(df_tavg_clean,"outputs/63_tavg_clean.csv", row.names = FALSE)
write.csv(df_tlr_clean,"outputs/63_tlr_clean.csv", row.names = FALSE)
```
