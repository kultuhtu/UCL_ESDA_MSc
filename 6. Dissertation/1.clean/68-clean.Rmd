...
df_heat <- read_csv(
  "0.data/sensor/home68_other756_sensor2115_tempprobe_central-heating-flow.csv.gz", show_col_types = FALSE)
df_gas <- read_csv(
  "0.data/sensor/home68_outside755_sensor2100_gas-pulse_gas.csv.gz", 
  show_col_types = FALSE)

df_tbath <- read_csv(
  "0.data/auxil/home68_bathroom763_sensor2157_room_temperature.csv.gz", show_col_types = FALSE)
df_tbed1 <- read_csv(
  "0.data/auxil/home68_bedroom754_sensor2097_room_temperature.csv.gz", show_col_types = FALSE)
df_tbed2 <- read_csv(
  "0.data/auxil/home68_bedroom762_sensor2152_room_temperature.csv.gz", show_col_types = FALSE)
df_tbed3 <- read_csv(
  "0.data/auxil/home68_bedroom764_sensor2162_room_temperature.csv.gz", show_col_types = FALSE)
df_thall <- read_csv(
  "0.data/auxil/home68_hall760_sensor2144_room_temperature.csv.gz", show_col_types = FALSE)
df_tkitch <- read_csv(
  "0.data/auxil/home68_kitchen759_sensor2139_room_temperature.csv.gz", show_col_types = FALSE)
df_tlr <- read_csv(
  "0.data/auxil/home68_livingroom761_sensor2149_room_temperature.csv.gz", show_col_types = FALSE)
df_tstu <- read_csv(
  "0.data/auxil/home68_study758_sensor2129_room_temperature.csv.gz", show_col_types = FALSE)
df_tur <- read_csv(
  "0.data/auxil/home68_utility757_sensor2134_room_temperature.csv.gz", show_col_types = FALSE)
...
# fix date format
df_tbed3_5 <- df_tbed3_5 %>%
  mutate(date = as.POSIXct(paste(date), format='%Y-%m-%d %H:%M:%S'))
df_tbed3_5 <- as_tbl_time(df_tbed3_5, date)
...
df_tstu_5 <- df_tstu_5 %>%
  mutate(date = as.POSIXct(paste(date), format='%Y-%m-%d %H:%M:%S'))
df_tstu_5 <- as_tbl_time(df_tstu_5, date)



# utility room to 5 min intervals
df_tur_5 <- df_tur %>%
  mutate(date = as.POSIXct(paste(date), format='%Y-%m-%d %H:%M:%S')) %>%
  group_by(date = cut(date, breaks="5 min")) %>%
  summarize(T = mean(T))
...
df_tur_5 <- df_tur_5 %>%
  mutate(date = as.POSIXct(paste(date), format='%Y-%m-%d %H:%M:%S'))
df_tur_5 <- as_tbl_time(df_tur_5, date)
```
...
df_tlr_filtered <- df_tlr_5 %>%
  filter_time('2017-01-10' ~ '2018-06-01')
df_tstu_filtered <- df_tstu_5 %>%
  filter_time('2017-01-10' ~ '2018-06-01')
df_tur_filtered <- df_tur_5 %>%
  filter_time('2017-01-10' ~ '2018-06-01')
```
...
df_gas_main <- as.data.table(df_gas_filtered)[as.data.table(df_nul), on = .(date), roll = -600]
...
df_tbed3_main <- as.data.table(df_tbed3_filtered)[as.data.table(df_nul), on = .(date), roll = TRUE]
df_tbed3_main <- as_tbl_time(df_tbed3_main, date)
...
df_tstu_main <- as.data.table(df_tstu_filtered)[as.data.table(df_nul), on = .(date), roll = TRUE]
df_tstu_main <- as_tbl_time(df_tstu_main, date)

df_tur_main <- as.data.table(df_tur_filtered)[as.data.table(df_nul), on = .(date), roll = TRUE]
df_tur_main <- as_tbl_time(df_tur_main, date)
...
sum(is.na(df_tbed3_main$T))
sum(is.na(df_tstu_main$T))
sum(is.na(df_tur_main$T))
...
```{r results = 'hold'}
df_tbath_clean <- df_tbath_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),186,.)))
df_tbed1_clean <- df_tbed1_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),185,.)))
df_tbed2_clean <- df_tbed2_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),192,.)))
df_tbed3_clean <- df_tbed3_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),186,.)))
df_tstu_clean <- df_tstu_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),157,.)))
df_tur_clean <- df_tur_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),165,.)))
df_thall_clean <- df_thall_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),189,.)))
df_tkitch_clean <- df_tkitch_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),211,.)))
df_tlr_clean <- df_tlr_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),199,.)))

sum(is.na(df_tbath_clean$T))
sum(is.na(df_tbed1_clean$T))
sum(is.na(df_tbed2_clean$T))
sum(is.na(df_tbed3_clean$T))
sum(is.na(df_tstu_clean$T))
sum(is.na(df_tur_clean$T))
...
df_tavg_clean$T <- (df_tbath_clean$T + df_tbed1_clean$T + df_tbed2_clean$T 
                     + df_thall_clean$T + df_tkitch_clean$T + df_tlr_clean$T
                    + df_tur_clean$T + df_tbed3_clean$T + df_tstu_clean$T)/6
df_tavg_clean
```
...
```{r}
# save main outputs
write.csv(df_gas_clean,"outputs/68_gas_clean.csv", row.names = FALSE)
write.csv(df_heat_clean,"outputs/68_heat_clean.csv", row.names = FALSE)
write.csv(df_tavg_clean,"outputs/68_tavg_clean.csv", row.names = FALSE)
write.csv(df_tlr_clean,"outputs/68_tlr_clean.csv", row.names = FALSE)
```
