...
```{r}
df_gas <- read_csv("outputs/63_gas_clean.csv", show_col_types = FALSE)
df_heat <- read_csv("outputs/63_heat_clean.csv", show_col_types = FALSE)
df_tlr <- read_csv("outputs/63_tlr_clean.csv", show_col_types = FALSE)
df_tavg <- read_csv("outputs/63_tavg_clean.csv", show_col_types = FALSE)
```
...
Classify heating as on for the sequence if the sum of the increment of the sequence is larger than 0.75 degrees C
...
```{r}
write.csv(df_main,"outputs/63_main.csv", row.names = FALSE)
```
