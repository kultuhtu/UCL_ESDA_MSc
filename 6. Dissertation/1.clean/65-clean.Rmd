...
df_heat <- read_csv(
  "0.data/sensor/home65_other729_sensor1893_tempprobe_central-heating-flow.csv.gz", show_col_types = FALSE)
df_gas <- read_csv(
  "0.data/sensor/home65_outside730_sensor1903_gas-pulse_gas.csv.gz", 
  show_col_types = FALSE)

df_tbath <- read_csv(
  "0.data/auxil/home65_bathroom734_sensor1954_room_temperature.csv.gz", show_col_types = FALSE)
df_tbed1 <- read_csv(
  "0.data/auxil/home65_bedroom736_sensor1939_room_temperature.csv.gz", show_col_types = FALSE)
df_tbed2 <- read_csv(
  "0.data/auxil/home65_bedroom743_sensor1942_room_temperature.csv.gz", show_col_types = FALSE)
df_thall <- read_csv(
  "0.data/auxil/home65_hall731_sensor1919_room_temperature.csv.gz", show_col_types = FALSE)
df_thall2 <- read_csv(
  "0.data/auxil/home65_hall735_sensor1932_room_temperature.csv.gz", show_col_types = FALSE)
df_tkitch <- read_csv(
  "0.data/auxil/home65_kitchen733_sensor1928_room_temperature.csv.gz", show_col_types = FALSE)
df_tlr <- read_csv(
  "0.data/auxil/home65_livingroom732_sensor1923_room_temperature.csv.gz", show_col_types = FALSE)
df_tpr <- read_csv(
  "0.data/auxil/home65_playroom744_sensor1946_room_temperature.csv.gz", show_col_types = FALSE)
...
colnames(df_thall2) <- c('date','T')
colnames(df_tkitch) <- c('date','T')
colnames(df_tlr) <- c('date','T')
colnames(df_tpr) <- c('date','T')
```
...
# hall2 to 5 min intervals
df_thall2_5 <- df_thall2 %>%
  mutate(date = as.POSIXct(paste(date), format='%Y-%m-%d %H:%M:%S')) %>%
  group_by(date = cut(date, breaks="5 min")) %>%
  summarize(T = mean(T))

# fix date format
df_thall2_5 <- df_thall2_5 %>%
  mutate(date = as.POSIXct(paste(date), format='%Y-%m-%d %H:%M:%S'))
df_thall2_5 <- as_tbl_time(df_thall2_5, date)
...
df_tpr_5 <- df_tpr_5 %>%
  mutate(date = as.POSIXct(paste(date), format='%Y-%m-%d %H:%M:%S'))
df_tpr_5 <- as_tbl_time(df_tpr_5, date)
```
...
df_thall_filtered <- df_thall_5 %>%
  filter_time('2017-01-10' ~ '2018-06-01')
df_thall2_filtered <- df_thall2_5 %>%
  filter_time('2017-01-10' ~ '2018-06-01')
df_tkitch_filtered <- df_tkitch_5 %>%
  filter_time('2017-01-10' ~ '2018-06-01')
df_tlr_filtered <- df_tlr_5 %>%
  filter_time('2017-01-10' ~ '2018-06-01')
df_tpr_filtered <- df_tpr_5 %>%
  filter_time('2017-01-10' ~ '2018-06-01')
```
...
df_gas_main <- as.data.table(df_gas_filtered)[as.data.table(df_nul), on = .(date), roll = TRUE]
...
df_thall2_main <- as.data.table(df_thall2_filtered)[as.data.table(df_nul), on = .(date), roll = TRUE]
df_thall2_main <- as_tbl_time(df_thall2_main, date)
...
df_tpr_main <- as.data.table(df_tpr_filtered)[as.data.table(df_nul), on = .(date), roll = TRUE]
df_tpr_main <- as_tbl_time(df_tpr_main, date)
...
sum(is.na(df_thall2_main$T))
sum(is.na(df_tkitch_main$T))
sum(is.na(df_tlr_main$T))
sum(is.na(df_tpr_main$T))
```
...
df_thall2_clean <- df_thall2_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),193,.)))
...
df_tavg_clean$T <- (df_tbath_clean$T + df_tbed1_clean$T + df_tbed2_clean$T 
                    ...
                    + df_thall2_clean$T + df_tpr_clean$T)/6
df_tavg_clean
```
...
```{r}
# save main outputs
write.csv(df_gas_clean,"outputs/65_gas_clean.csv", row.names = FALSE)
write.csv(df_heat_clean,"outputs/65_heat_clean.csv", row.names = FALSE)
write.csv(df_tavg_clean,"outputs/65_tavg_clean.csv", row.names = FALSE)
write.csv(df_tlr_clean,"outputs/65_tlr_clean.csv", row.names = FALSE)
```
