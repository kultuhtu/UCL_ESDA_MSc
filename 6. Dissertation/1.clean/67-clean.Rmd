...
df_heat <- read_csv(
  "0.data/sensor/home67_livingroom766_sensor2234_tempprobe_central-heating-flow.csv.gz", show_col_types = FALSE)
df_gas <- read_csv(
  "0.data/sensor/home67_outside765_sensor2222_gas-pulse_gas.csv.gz", 
  show_col_types = FALSE)

df_tbath <- read_csv(
  "0.data/auxil/home67_bathroom769_sensor2254_room_temperature.csv.gz", show_col_types = FALSE)
df_tbed1 <- read_csv(
  "0.data/auxil/home67_bedroom768_sensor2250_room_temperature.csv.gz", show_col_types = FALSE)
df_tbed2 <- read_csv(
  "0.data/auxil/home67_bedroom770_sensor2260_room_temperature.csv.gz", show_col_types = FALSE)
df_tkitch <- read_csv(
  "0.data/auxil/home67_kitchen771_sensor2274_room_temperature.csv.gz", show_col_types = FALSE)
df_tlr <- read_csv(
  "0.data/auxil/home67_livingroom766_sensor2265_room_temperature.csv.gz", show_col_types = FALSE)
...
```{r results = 'hold'}
df_tbath_clean <- df_tbath_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),196,.)))
df_tbed1_clean <- df_tbed1_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),185,.)))
df_tbed2_clean <- df_tbed2_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),217,.)))
df_tkitch_clean <- df_tkitch_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),204,.)))
df_tlr_clean <- df_tlr_main %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),213,.)))
...
```{r}
# save main outputs
write.csv(df_gas_clean,"outputs/67_gas_clean.csv", row.names = FALSE)
write.csv(df_heat_clean,"outputs/67_heat_clean.csv", row.names = FALSE)
write.csv(df_tavg_clean,"outputs/67_tavg_clean.csv", row.names = FALSE)
write.csv(df_tlr_clean,"outputs/67_tlr_clean.csv", row.names = FALSE)
```
